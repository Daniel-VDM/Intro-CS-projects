...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        def remove_fake_queen():
            """Removes the a fake queen if that is the case"""
            if not insect.real_queen:
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 11
        return self.container == True and other.container == False and self.ant == None or False
        # END Problem 11
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for _ in range(self.min_range):
            if place.entrance and place.entrance != hive:
                step_count += 1
                place = place.entrance
        # Then check the rest
        while place.entrance and place.entrance != hive and len(place.bees) == 0:
            place = place.entrance
            step_count += 1

        if self.min_range <= step_count <= self.max_range:
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            ...
            for bee in bee_lst:
                ...
                # Personal Note: When a bee is killed, it gets AUTOMATICALLY 
                # removed from whatever place it was in, so no need to change place.bees
...
# END Problem 13
    ...
    # END Problem 13
    food_cost = 7
    real_queen_exists = False
    buffed_ants = []

    def __init__(self):
        # BEGIN Problem 13
        Ant.__init__(self, 1)
        # Only one queen can exist
        if QueenAnt.real_queen_exists:
            self.real_queen = False
        else:
            self.real_queen = True
            QueenAnt.real_queen_exists = True
            self.buffed_ants.append(self)   # Queen ants are never buffed
        # END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        # Remove queen if it is an impostor
        ...
        # Buff ants Logic
        def buff(ant, buffed_list):
            if ant not in buffed_list:
                ant.damage *= 2
                buffed_list.append(ant)            

        place = self.place
        while place: 
            if place.ant:      #Test if place has an ant, then do logic
                if place.ant.container and place.ant.ant:
                    buff(place.ant.ant, self.buffed_ants)
                buff(place.ant,self.buffed_ants)
            place = place.exit
        # END Problem 13
...
def make_slow(action):
    ...
    return slow, 'slow'         # Return a tuple for effect identification
    # END Problem EC
...
def make_stun(action):
    ...
    def stunned(bee, colony):
        pass
    return stunned, 'stun'      # Return a tuple for effect identification
    # END Problem EC

def apply_effect(effect, bee, duration):
    """Apply a status effect to a BEE that lasts for DURATION turns."""
    # BEGIN Problem EC
    curr_effect, effect_name = effect(Bee.action)
    # Check for status attrs and adds them if bee doesnt have one
    if not hasattr(bee, "effects") and not hasattr(bee, "original_action"):
        bee.effects = {'stun' : [], 'slow' : []} 
        bee.original_action = bee.action  

    bee.effects[effect_name].append(duration)

    # Replacement for bee actions (which is why bees now have attrs to keep track for default actions)
    def status_action(colony):
        nonlocal curr_effect, effect_name
        curr_effect(bee,colony)

        # create a copy of stack duration list incase anything gets removed while processing       
        duration_list = list(bee.effects[effect_name])
        # Takes care of slow stacks duration logic
        if effect_name == 'slow' and len(duration_list) > 1 and colony.time % 2 == 0:
            for i in range(1, len(duration_list)):
                duration_list[i] -=1
        # Decrement duration of current stack by 1 at each action
        duration_list[0] -= 1
        # Remove any stack that is at 0 duration, and rebind to dict
        bee.effects[effect_name] = [x for x in duration_list if x > 0]

        # If BOTH effects are done, return bee to original effect
        if bee.effects['slow'] == [] and bee.effects['stun'] == []: 
            bee.action = bee.original_action
        # Check individual effect, Stun has priority over slow
        elif bee.effects['stun'] == []:
            curr_effect, effect_name = make_slow(Bee.action)
        else:
            curr_effect, effect_name = make_stun(Bee.action)
