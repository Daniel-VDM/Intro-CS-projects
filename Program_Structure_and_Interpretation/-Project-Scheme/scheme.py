...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    >>> expr = read_line('(+ 2 2)')
    >>> expr
    Pair('+', Pair(2, Pair(2, nil)))scheme_atomp(expr)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals != vals:
            raise SchemeError("number of arguments does not match number of formal params");
        # END PROBLEM 11
        return child
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Type error of', e)
        # END PROBLEM 4
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        while operands != nil and formals != nil:
        	self.env.define(formals.first, operands.first)
        	operands, formals = operands.second, formals.second
        if formals != operands:
        	raise SchemeError("Miss Matched Parameters for", self)

        # not sure if all "complete_eval"s are needed here 
        evaled_body = self.body.map( lambda x: complete_eval(scheme_eval(x, self.env)) )
       	return complete_eval( eval_all(evaled_body, env) )
        # END Problem 21
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        ...
        if expressions.second != nil:
            prev = scheme_eval(expressions.first, env) 
        else:
            prev = scheme_eval(expressions.first, env, True)
        if not scheme_truep(prev):
            return False
        expressions = expressions.second
    return prev
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        bind = bindings.first
        if len(bind) != 2:
            raise SchemeError("Mal formed bindings")
        check_formals(Pair(bind.first, nil))
        new_env.define(bind.first, scheme_eval(bind.second.first, env))
...
def do_define_macro(expressions, env):
    ...
    if isinstance(expressions.first, Pair):
        ...
        # Not too sure about this exception condition (maybe we dont need the last 2 predicates)
        if not scheme_numberp(name) and not scheme_booleanp(name) and not scheme_nullp(name):
