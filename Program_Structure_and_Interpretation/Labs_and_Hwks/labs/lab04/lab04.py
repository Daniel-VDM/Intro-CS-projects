...
def closer_city(lat, lon, city1, city2):
    ...
    # Then set distances 
    distance_to_city1 = distance(location,city1)
    distance_to_city2 = distance(location,city2)
...
def replace_elem(lst, index, elem):
    ...
    # recall list splitting for agruments lst[: 2] gives u element 0 and 1 and lst[2 :] gives you element 2 till last
...
def put_piece(board, max_rows, column, player):
    ...
    -1
    """
    # check first row for full column
    if board[0][column] != '-':
        return -1, board

    rows = max_rows-1

    # check for eveything except first row
    for i in range(rows):
        #check row below for X/O
        if board[i+1][column] in ['X','O']:
            board[i] = replace_elem(board[i], column, player)
            return i, board

    # if everything else is passed(not executed), then it must be an empty column, so just replace last row in column
    board[rows] = replace_elem(board[rows], column, player)
    return rows , board
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    -1
    """
    # handles invalid column
    if col < 0 or col >= max_cols:
        return -1, board

    row_index, new_board = put_piece(board, max_rows, col, player)
    
    # handles full column (so invalid move)
    if row_index == -1:
        return -1, board

    # return the board with its move done and the row it was added to
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for j in range(max_cols-1):
        ##->remeber to preserve abstraction barriers<-##
        if get_piece(board,row,j) == get_piece(board,row,j+1) and get_piece(board,row,j) == player:
            connects += 1
        else:
            connects = 0
        if connects == num_connect-1:   # num_connects-1 because of how the logic is implemented. 
            return True                 # 1 'X'or'O' by itself would be considered 1 connection. Just shift to compensate.
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    for i in range(max_rows-1):
        ##->remeber to preserve abstraction barriers<-##
        if get_piece(board,i,col) == get_piece(board,i+1,col) and get_piece(board,i,col) == player:
            connects += 1
        else:
            connects = 0
        if connects == num_connect-1:
            return True
    return False
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    True
    """
    diagonal_win = lambda: check_win_diagonal(board, max_rows, max_cols, 
                                              num_connect, row, col, player)
    row_win = lambda: check_win_row(board, max_rows, max_cols, num_connect,
                                    row, player)
    col_win = lambda: check_win_column(board, max_rows, max_cols, num_connect,
                                       row, player)
    # Function calls b/c if it short-circuits to True.
    # Any further function call is useless. Just optimization stuffs~
