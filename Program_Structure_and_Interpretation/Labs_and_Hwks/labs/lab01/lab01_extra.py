...
def falling(n, k):
    ...
    f_fact = 1
    while k:
        f_fact, n , k = n * f_fact, n-1, k-1    #remeber that in a while loop the boolean context needs to come to a false condition
    return f_fact
...
def guess_linear():
    """Guess in increasing order and return the number of guesse"""
    prompt_for_number(LOWER, UPPER)
    num_guesses, guess= 1, LOWER
    
    while guess <= UPPER: #keeps guesses within the domain that the user could have chosen from
        if is_correct(guess):
            return num_guesses
...
def guess_binary():
    ...
    """
    prompt_for_number(LOWER, UPPER)
    lower, upper, num_guesses = LOWER, UPPER, 1
    
    while lower <= upper:           #keep bounds within the domain that the user can choose from
        guess = (lower + upper) // 2
        if is_correct(guess):       
            return num_guesses
        elif is_too_high(guess):    #if guess is too high, you look in smaller partition by setting bounds
            upper = guess - 1
        else:                       #else guees has to be too low, you look in larger partition by setting bounds
            lower = guess + 1
        num_guesses += 1
