...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        person_obj = self.place.characters.get(person, None)
        if person_obj:
            print(person, 'says:', person_obj.talk())
        else:
    ...
    def take(self, thing):
        ...
        thing_obj = self.place.things.get(thing, None)
        if thing_obj:
            self.backpack.append(thing_obj)
            self.place.take(thing) #dictionary .pop(-arg-) method has to have non-mutables for -arg-. so pop returns hashtable value of arg 
            print(self.name, 'takes the', thing_obj.name)
        else:
    ...
    def unlock(self, place):
        ...
        dest = self.place.get_neighbor(place)
        if key:
            if dest.locked == True:
